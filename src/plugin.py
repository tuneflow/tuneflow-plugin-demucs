...
class MusicSourceSeparatePlugin(TuneflowPlugin):
    ...
    def run(song: Song, params: dict[str, Any]):
        ...
        audio_clip_data: song_pb2.AudioClipData = MusicSourceSeparatePlugin._get_selected_audio_clip_data(song, params)
        MusicSourceSeparatePlugin._separate_music_sources(song, audio_clip_data.audio_file_path, audio_clip_data.duration)
    ...
    def _separate_music_sources(song: Song, audio_file_path, duration):
        ...
        print("Start separating music source...")
        subprocess.run([sys.executable, "-m", "demucs", "-d", "cpu", "--out", output_dir_path.absolute(), audio_file_path], check=True)
        print("Completed separating music source.")

        output_audio_file_paths = []
        output_audio_file_paths.append(output_dir_path.joinpath("htdemucs").joinpath(file_path.stem).joinpath("vocals.wav"))
        output_audio_file_paths.append(output_dir_path.joinpath("htdemucs").joinpath(file_path.stem).joinpath("drums.wav"))
        output_audio_file_paths.append(output_dir_path.joinpath("htdemucs").joinpath(file_path.stem).joinpath("bass.wav"))
        output_audio_file_paths.append(output_dir_path.joinpath("htdemucs").joinpath(file_path.stem).joinpath("other.wav"))

        for output_audio_file_path in output_audio_file_paths:
            MusicSourceSeparatePlugin._create_track(song, str(output_audio_file_path.absolute()), duration) 
